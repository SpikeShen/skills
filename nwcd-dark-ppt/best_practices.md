...
当使用大字号（36pt+）展示统计数字、关键指标、日期等视觉焦点元素时，必须确保文本框宽度足够容纳完整内容，避免换行。换行会破坏视觉冲击力和排版节奏。

### 规则
- 大数字文本框宽度应根据内容长度动态计算，而非使用固定宽度
- 短内容（1-3字符）可用较窄文本框，长内容（5+字符）需加宽
- 如果卡片宽度有限，优先缩小字号而非允许换行
- 统计数字、日期、百分比等焦点元素绝不应出现换行

```python
# ✅ 正确：根据内容长度调整文本框宽度或字号
def add_stat_number(slide, number, label, left, top, card_width, ...):
    # 根据字符数估算所需宽度，确保不换行
    text_width = max(card_width - 0.3, 1.5)
    # 如果内容较长，适当缩小字号
    font_size = 48 if len(number) <= 4 else 40

# ❌ 错误：所有统计数字使用相同固定宽度的文本框
# 导致 "2017.12" 这样的内容换行，破坏视觉效果
```
